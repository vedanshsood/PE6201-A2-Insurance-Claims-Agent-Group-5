...
Applying the "try not adding a tool" heuristic, we rejected creating a 7th tool (`lookup_required_documents`) to resolve V1’s failure on `CLM-8901`. Adding another tool definition would have expanded prompt prefix overhead ($B$, Cost Lever 1) by ~150 tokens every turn and added sequential turns. Instead, we expanded `check_coverage` to surface `required_document` directly. This restraint bounded prefix costs, maintained tool discriminability, and converted `CLM-8901` to a 100% pass rate in V2 across all 3 trials.
